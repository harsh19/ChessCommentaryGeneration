...
def getThreatFeatures(board, prev_board, current_player, other_player, from_position, to_position):
    ...
    #print  "pieces = ", pieces
    #vals = getMoveNamesToFeatures(pieces)
    '''for val,piece in zip(vals,PIECE_TYPES):
        if val==1:
            ret.append(typ + 'true')
        else:
            ret.append(typ + 'false')
    '''
    #ret.extend( getMoveNamesToFeatures(pieces) )
    typ = "beforethreats_"
    default = "none"
    for i,piece in enumerate(pieces):
        ret.append(typ+piece)
        if i>=5:
            break
    if len(pieces)<6:
        for i in range(6-len(pieces)):
            ret.append(typ+default)
...
def main(src_dir, src, typ="threat"):
    ...
    for line in che_data:
        ...
        #print len(threat_feats)
        assert len(threat_feats)==18
