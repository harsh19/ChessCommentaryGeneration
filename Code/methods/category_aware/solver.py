...
class Solver:
    ...
    def main(self):
        ...
        if cnfg.mode!="inference":
            valid_src=srcLangObj.read_corpus(mode="valid", typ="sequence_category")
        ...
        train_src_batches,train_src_masks=torch_utils.splitBatches(train=train_src,batch_size=cnfg.batch_size,padSymbol=cnfg.garbage,method="pre",typ="sequence_category")
        ...
        if cnfg.mode!="inference":
            valid_src_batches,valid_src_masks=torch_utils.splitBatches(train=valid_src,batch_size=cnfg.batch_size,padSymbol=cnfg.garbage,method="pre",typ="sequence_category")
        ...
        test_src_batches,test_src_masks=torch_utils.splitBatches(train=test_src,batch_size=1,padSymbol=cnfg.garbage,method="pre",typ="sequence_category")
        ...
        if cnfg.mode=="train" or cnfg.mode=="LM":
            ...
            if cnfg.method=="OSOM":
                ...
                for i in range(len(test_src)):
                    ...
                    testIndex[i]=list(zip(train_src_batches,train_tgt_batches,train_src_masks,train_tgt_masks))
                    #print testIndex[i][0]
                    #print testIndex[i][1]
