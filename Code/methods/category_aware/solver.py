...
class Solver:
    def __init__(self, cnfg):
        ...
        self.cnfg.srcLangObj = self.cnfg.readData.Lang(
            self.cnfg.srcLang,
            self.cnfg.min_src_frequency,
            taskInfo=cnfg.taskInfo,
        )
        self.cnfg.tgtLangObj = self.cnfg.readData.Lang(
            self.cnfg.tgtLang,
            self.cnfg.min_tgt_frequency,
            taskInfo=cnfg.taskInfo,
        )
    ...
    def main(self):
        ...
        train_src = srcLangObj.read_corpus("train", typ="sequence_category")
        train_tgt = tgtLangObj.read_corpus("train")

        if cnfg.mode != "inference":
            valid_src = srcLangObj.read_corpus(
                mode="valid", typ="sequence_category"
            )
            valid_tgt = tgtLangObj.read_corpus(mode="valid")

        test_src = srcLangObj.read_corpus(mode="test", typ="sequence_category")
        test_tgt = tgtLangObj.read_corpus(mode="test")

        train_src, train_tgt = (
            train_src[: cnfg.max_train_sentences],
            train_tgt[: cnfg.max_train_sentences],
        )
        ...
        test_tgt_batches, test_tgt_masks = torch_utils.splitBatches(
            train=test_tgt, batch_size=1, padSymbol=cnfg.garbage, method="post"
        )
        ...
        if cnfg.mode == "trial":
            ...
            print(("Source Batch Shape:", train_src_batches[30][0].shape))
        ...
        if cnfg.mode == "train" or cnfg.mode == "LM":
            ...
            for epochId in range(cnfg.NUM_EPOCHS):
                ...
                for batchId, batch in enumerate(train_batches):
                    src_batch, tgt_batch, src_mask, tgt_mask = (
                        batch[0],
                        batch[1],
                        batch[2],
                        batch[3],
                    )
                    batchLength = src_batch[0].shape[1]
                    batchSize = src_batch[0].shape[0]
                    tgtBatchLength = tgt_batch.shape[1]
                    if (
                        batchLength < cnfg.MAX_SEQ_LEN
                        and batchSize > 1
                        and tgtBatchLength < cnfg.MAX_TGT_SEQ_LEN
                    ):
                        model.zero_grad()
                        loss = model.forward(
                            src_batch,
                ...
                model.save_checkpoint(
                    modelName + "_" + str(epochId), optimizer
                )
            ...
            encodeVocab = {}
            encodeReverseVocab = {}
            for key, val in list(model.wids_src.items()):
                encodeVocab[key] = val
                encodeReverseVocab[val] = key
            decodeVocab = {}
            decodeReverseVocab = {}
            for key, val in list(model.wids_tgt.items()):
                decodeVocab[key] = val
