...
class AttnDecoderRNN(nn.Module):
    def __init__(self,wids,vocabSize,emb_size,hidden_size,use_LSTM,use_attention,share_embeddings,reference_embeddings=None,sigmoid=False,category_feature_size=1):
        ...
        if self.use_attention:
            if self.use_LSTM:
                self.decoder=nn.LSTM(self.emb_size+self.hidden_size + category_feature_size, self.hidden_size)
            else:
                self.decoder=nn.GRU(self.emb_size+self.hidden_size + category_feature_size, self.hidden_size)
        else:
            if self.use_LSTM:
                self.decoder=nn.LSTM(self.emb_size+category_feature_size,self.hidden_size)
            else:
                self.decoder=nn.GRU(self.emb_size+category_feature_size,self.hidden_size)

    def forward(self,batchSize,tgtEmbedIndex,encoderOutTensor,o_t,hidden,feedContextVector=False,contextVector=None,inference=False,getAtt=False,categories=None):
        ...
        #print "before: tgtEmbeds = ", tgtEmbeds
        categories = categories.view(1,batchSize,self.category_feature_size)
        tgtEmbeds=torch.cat([tgtEmbeds,categories],2).view(1,batchSize,-1)
        #print "after: tgtEmbeds = ", tgtEmbeds
