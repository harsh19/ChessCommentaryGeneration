...
inSrcFile=open(split+".che-eng.1.che")
outSrcFile=open(split+".che-eng.1attack.che","w")

inTgtFile=open(split+".che-eng.1.en")
outTgtFile=open(split+".che-eng.1attack.en","w")
...
for line in inSrcFile:
    ...
    currentBoardFENEstimated=convertToFEN(currentBoard,moveColor)
    ...
    """
    try:
        score1=boardInfoObj.getScore(previousBoardFEN)
        score2=boardInfoObj.getScore(currentBoardFENEstimated)
        scoreDiff=score2.cp-score1.cp
        if scoreDiff>0:
            scoreDiff="P"
        else:
            scoreDiff="N"
        ...
        scoreDiff="U"
    scoreDiffString=str(scoreDiff)+" <EOScore>"
    """
