...
class Lang:
    def __init__(
        self, name, min_frequency, dataPath="data/", taskInfo="en-de.low"
    ):
        self.name = name
    ...
    def read_corpus(
        self, mode, typ="sequence"
    ):  # type {sequence, three_tuple}
        fileName = self.dataPath + ".".join([mode, self.taskInfo, self.name])
        sentences = []
        ...
        if typ == "sequence":
            for line in open(fileName):
                ...
                words.append("</s>")
                sentence = [
                    self.wids[word] if word in self.wids else 1
                    for word in words
                ]
            ...
            for line in open(fileName):
                ...
                for component in components:
                    ...
                    sentence = [
                        self.wids[word]
                        if word in self.wids
                        else self.wids[self.unk_word]
                        for word in words
                    ]
                    tmp.append(sentence)
...
def read_corpus(
    wids, mode="train", update_dict=True, min_frequency=3, language="en"
):
    fileName = None
    if mode == "train":
